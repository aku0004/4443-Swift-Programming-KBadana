...
class ViewController: UIViewController {
    ...
    //["Red","Green","Blue","Orange","Black","White","Brown","Purple","Gray","Yellow"]
    
    //Color dictionary type not used in the program but kept as an example for future
    var colorRGB = Dictionary<String,UIColor>()
    /*= [
        ["Red":0xFF0000],
        ["Green":0x00FF00],
        ["Blue":0x0000FF],
            ...
            .responseJSON { (req, res, json, error) in
                if(error != nil) {
                   ...
                   // }
                    ...
                    for (key,val) in json{
                        println("\(key):\(val)")
                        self.colorNames.append(key)
                        println(val.string!)
                        //Temp=self.hexStringToUIColor(val.string!)
                        //self.colorRGB[key]=Temp
                        //println("\(key):\(self.hexStringToUIColor(val.string!))")
                        self.colorRGB["\(key)"]=self.hexStringToUIColor(val.string!)
                       // println("#############")
                        //println(TempVal)
                        //println("##############")
                        //self.colorRGB[key]=colorWithHexString(val.description)
                        // pushing the values into the array
                    }
                    println("**********************")
                    println(self.colorNames.count)
                    println(self.colorRGB.count)
                    println("**********************")
    ...
    func matchColorLogic() -> Bool{
       ...
       // println(buttonAnswer)
        ...
        colorNumber = Int(arc4random_uniform(UInt32(colorNames.count)))
    ...
    func changeColor() -> Bool{
        ...
        var ColName=colorNames[colorNumber]
        var ColorCode=colorRGB[ColName]
        lblChangeColor.backgroundColor=ColorCode
        /*
        if(colorNumber == 0){
